...
SetTimer, SprintOff, Delete
SetTimer, SprintOn, Delete
...
; ########### DayZ ############
#IfWinActive ahk_exe DayZ_x64.exe


    ~*Right::
        global autorun := not autorun
        if autorun
        {
            SetTimer, SprintOff, Off
            SetTimer, SprintOn, 10
        } else  {
            SetTimer, SprintOff, Off
            SetTimer, SprintOn, Off
            SendInput {blind}{LShift up}
            SendInput {blind}{w up}
        }
    return
...
SprintOn:
    IfWinNotExist, ahk_exe DayZ_x64.exe
    {
        SetTimer, SprintOn, Off
        SetTimer, SprintOff, Off
        return
    ...
    SetTimer, SprintOff, 5000
return

SprintOff:
    SetTimer, SprintOff, Off
    SendInput {blind}{LShift up}
    SetTimer, SprintOn, 4900
return
